...
public class DeplacementRotation implements Deplacement {
	...
	public int[][] deplacement(int[][] positionsPrecedentes, TypeBloc piece) {
		int[][] res = new int[4][2];
		if(piece.getCouleur().equals(TypeBloc.Carre.getCouleur()) || piece.getCouleur().equals(TypeBloc.Ligne.getCouleur())) {
		    int[][] nouvellesCoordonnees = new int[positionsPrecedentes.length][2];

		    for (int i = 0; i < positionsPrecedentes.length; i++) {
		        int x = positionsPrecedentes[i][0];
		        int y = positionsPrecedentes[i][1];
